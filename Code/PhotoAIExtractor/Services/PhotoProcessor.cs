        ...
        await Parallel.ForEachAsync(imageFiles, parallelOptions, async (filePath, ct) =>
        {
            ...
            try
            {
                ...
                if (shouldOptimize && optimizedBaseFolder != null)
                {
                    ...
                    {
                        // Use the hash-based filename generated by ImageOptimizer
                        var optimizedPath = optimizationResult.OptimizedPath;
                        var optimizedFileName = Path.GetFileName(optimizedPath);
                        ...
                        {
                            WebPImage = MakeRelativePath(folderPath, optimizedPath),
                        ...
                        logger.LogInformation(
                            "Optimized: {FileName} → {DateFolder}/{LocationFolder}/{OptimizedFileName} ({OriginalSize} → {OptimizedSize}, {CompressionRatio:F1}% saved)",
